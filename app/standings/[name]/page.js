...
function getOpponentName(game, teamId) {
  const isHome = game.home_team_id === teamId;
  return isHome ? game.away_team?.name || "Opponent" : game.home_team?.name || "Opponent";
}
...
export default async function TeamPage({ params }) {
  ...
  const team = (allTeams || []).find((item) => slugifyTeamName(item.name) === teamSlug);
  ...
  // Assumes common schema: players table contains per-player season stats.
  // If your stats live in a different table, this section is the only part you may need to adjust.
  ...
  if (playersError) {
    playerStatsError =
      "Could not load player stats. The team page works, but your stats query may need to be adjusted to your exact Supabase schema.";
  } else {
      ...
      .map((player) => ({
        ...
        points:
          player.points !== null && player.points !== undefined
            ...
            : Number(player.goals || 0) + Number(player.assists || 0),
  ...
  return (
    <main style={pageWrap}>
      <div style={shell}>
        ...
        <section style={card}>
          <h1 style={{ fontSize: 40, marginTop: 0, marginBottom: 8 }}>{team.name}</h1>
          <p style={{ color: "#94a3b8", marginTop: 0, marginBottom: 24 }}>
            Team results and player stats
          </p>

          {gamesError ? (
            <p style={{ color: "#fca5a5" }}>Could not load results: {gamesError.message}</p>
          ) : (
            <>
              <div
                style={{
              ...
              >
                <div style={subCard}>
                  <div style={{ color: "#94a3b8", fontSize: 13, marginBottom: 6 }}>Games Played</div>
                  <div style={{ fontSize: 28, fontWeight: 800 }}>{games.length}</div>
                </div>

                <div style={subCard}>
                  <div style={{ color: "#94a3b8", fontSize: 13, marginBottom: 6 }}>Wins</div>
                  <div style={{ fontSize: 28, fontWeight: 800, color: "#67e8f9" }}>{teamWins}</div>
                </div>

                <div style={subCard}>
                  <div style={{ color: "#94a3b8", fontSize: 13, marginBottom: 6 }}>Losses</div>
                  <div style={{ fontSize: 28, fontWeight: 800 }}>{teamLosses}</div>
                </div>

                <div style={subCard}>
                  <div style={{ color: "#94a3b8", fontSize: 13, marginBottom: 6 }}>OTL</div>
                  <div style={{ fontSize: 28, fontWeight: 800 }}>{teamOTL}</div>
                </div>

                <div style={subCard}>
                  <div style={{ color: "#94a3b8", fontSize: 13, marginBottom: 6 }}>Ties</div>
              ...
              <div style={{ marginBottom: 32 }}>
                ...
                {games.length === 0 ? (
                  <p style={{ color: "#cbd5e1" }}>No final games posted yet for this team.</p>
                ) : (
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      ...
                      <tbody>
                        {games.map((game) => {
                          ...
                          return (
                            <tr key={game.id}>
                              <td style={tdStyle}>{formatDisplayDate(game.game_date)}</td>
                              ...
                              <td style={tdStyle}>{getTeamScoreDisplay(game, team.id)}</td>
                              <td style={tdStyle}>
                                {game.result_type
                                  ? String(game.result_type).replace(/^./, (m) => m.toUpperCase())
                                  : "Regulation"}
                              </td>
                            </tr>
                          );
              ...
              <div>
                ...
                {playerStatsError ? (
                  ...
                  <p style={{ color: "#cbd5e1" }}>No player stats posted yet for this team.</p>
                ) : (
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      <thead>
